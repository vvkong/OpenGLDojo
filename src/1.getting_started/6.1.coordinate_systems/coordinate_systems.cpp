...
bool doInitBeforeGLRenderLoop() {
    gdShader = new GDShader("6.1.shader.vs", "6.1.shader.fs");
...
void onRender() {
    ...
    glm::mat4 model = glm::rotate(oneMat, -3.14f/3.0f, glm::vec3(1.0f, 0.0f, 0.0f));
    glUniformMatrix4fv(glGetUniformLocation(gdShader->getShaderProgram(), "model"), 1, GL_FALSE, glm::value_ptr(model));

    glm::mat4 view = glm::translate(oneMat, glm::vec3(0.0f, 0.0f, -3.0f));
